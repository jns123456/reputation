...
def get_cached_top_predictors(*, category_slug="", limit=50):
    from accounts.category_selectors import get_top_predictors_by_category
    from accounts.selectors import get_top_predictors

    cache_key = _cache_key(kind="rep", category_slug=category_slug, limit=limit)
    leaders = cache.get(cache_key)
    if leaders is not None:
        return leaders

    if category_slug:
        leaders = list(get_top_predictors_by_category(category_slug, limit))
    else:
