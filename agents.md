...
[2026-05] domain — Forecast gating must NOT trust the imported `status` alone (Polymarket sync lags 30min–6h, so a closed/decided event can stay locally `OPEN` → exploitable). `Market.is_forecastable` = `is_open and not is_expired and accepting_orders and not is_in_play`; `is_expired` = `close_date <= now()`; `is_in_play` = `game_start_time <= now()` (local, network-free backstop so a started live event — tennis/soccer with uncertain end — closes forecasts even if the synced source flag is stale). To replicate Polymarket for live events (tennis/soccer end at uncertain times), we capture `acceptingOrders` (real-time order gate) + `gameStartTime` per market/sub-market in normalization (`_accepts_orders`/`_any_accepts_orders`), persisted as `Market.accepting_orders`/`game_start_time`. `create_prediction`/`exit_prediction`/`ForecastForm`/detail templates all gate on this (never `is_open` alone). `accepting_orders` defaults True (missing flag / Kalshi unaffected).
