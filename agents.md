...
[2026-05] integration — Grouped Polymarket winners may be `automaticallyResolved` with `outcomePrices` `["1","0"]` but no `resolvedOutcome`; `_market_is_resolved_yes` must infer Yes from prices (≥0.99). Stale refresh must include `closed` markets with empty `resolved_outcome`. **3-way soccer:** `is_soccer_match_event` must count all moneyline legs (`open_only=False`); requiring only open legs made post-match refresh/normalize return None, leaving pending forecasts stuck while UI odds hit 100%.
```
