...
[2026-07] integration — Heroku Essential-0 hit 94% disk: `markets_market` TOAST (~841 MB) from `polymarket_event_raw` on ~32k resolved rows. Keep disk bounded with (1) `delete_orphan_resolved_markets` / beat task (`MARKET_ORPHAN_RESOLVED_*`, default retain 30d, never delete rows with predictions/comments/challenges/watches), (2) compact raw on resolve + nightly `prune_market_raw`, (3) `VACUUM FULL markets_market` after large deletes (may need brief plan upgrade if free space is too low for rewrite).
```

---

*Last updated: 2026-07-15. Update §1–17 when architecture, scope, or conventions change; update §18 when durable lessons are learned or retired.*
