...
[2026-05] testing — `manage.py test` uses LocMem cache (`_RUNNING_TESTS` in settings) so Redis need not run locally; `create_user()` defaults `onboarding_completed=True` to satisfy `ProfileSetupRequiredMiddleware`.
