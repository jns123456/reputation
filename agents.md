...
[2026-05] workflow — to check whether failing tests are pre-existing, do NOT `git stash` the whole tree (untracked files + `-u` tangle pops, risking lost uncommitted work). Instead run the suite on a clean `git worktree`, or trust that unrelated failures (avatar/bookmarks/user_search return 302) come from the onboarding/email gating middlewares with non-onboarded test users.
...
[2026-05] testing — ~28 view-layer tests fail pre-existing (POST returns 302 != 200, e.g. forum vote/repost/bookmark). Environmental, NOT your change. Verify your delta with a clean baseline: `git stash -u` (untracked included), run tests, `git stash pop` — plain `git stash` skips new files and gives a false baseline.
